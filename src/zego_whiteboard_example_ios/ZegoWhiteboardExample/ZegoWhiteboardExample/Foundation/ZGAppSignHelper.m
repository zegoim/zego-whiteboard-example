...
+ (NSData *)converAppSignStringToData:(NSString *)appSignString {
    if(appSignString == nil || appSignString.length == 0) {
        return nil;
    }
...
//    appSignString = [appSignString stringByReplacingOccurrencesOfString:@"0x" withString:@""];
    ...
    for (int i = 0; i < appSignString.length/2.0 ; i++) {
        NSString *subString = [appSignString substringWithRange:NSMakeRange(i * 2, 2)];
        [bytes addObject:subString];
    }
